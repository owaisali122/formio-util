...
export interface ReactTransStackTableSchema {
  display?: 'form' | 'wizard'
  components?: unknown[]
  [key: string]: unknown
}

export interface ReactTransStackTableProps {
  /** Form.io JSON schema (from builder) */
  schema: ReactTransStackTableSchema
  ...
  if (!registrationPromise) {
    registrationPromise = registerCustomComponents().then(() => {
      ...
      if (!FormioGlobal || typeof FormioGlobal.createForm !== 'function') {
        throw new Error('[ReactTransStackTable] Formio.createForm is not available after registration')
      }
...
export function ReactTransStackTable(props: ReactTransStackTableProps) {
  ...
  useEffect(() => {
    ...
    ensureComponentsRegistered()
      ...
      .catch((err) => {
        if (!cancelled) console.error('[ReactTransStackTable] createForm error:', err)
      })
